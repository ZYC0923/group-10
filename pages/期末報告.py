...
st.title("第10組 機械工程實驗(二)熱流力實驗 期末報告")

st.write("這裡是熱流力實驗期末報告的詳細內容。")

# 設定文件路徑
file_paths = {
    "docx": "files/第10組_機械工程實驗(二)熱流力實驗 期末報告 .docx",  # 熱流力實驗 DOCX
    "pdf": "files/第10組_機械工程實驗(二)熱流力實驗 期末報告 .pdf",  # 熱流力實驗 PDF
}
...
for file_type, path in file_paths.items():
    if os.path.exists(path):  # 檢查文件是否存在
        ...
        if file_type == "pdf":
            ...
            label = "下載 第10組_機械工程實驗(二)熱流力實驗 期末報告 .pdf"
        elif file_type == "docx":
            mime_type = "application/vnd.openxmlformats-officedocument.wordprocessingml.document"
            label = "下載 第10組_機械工程實驗(二)熱流力實驗 期末報告 .docx"
...
if "pdf" in file_paths and os.path.exists(file_paths["pdf"]):
    st.subheader("第10組_機械工程實驗(二)熱流力實驗 期末報告 .pdf 內容")
